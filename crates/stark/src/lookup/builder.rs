...
use super::LookupInteraction;

/// A builder for the lookup table interactions.
pub struct LookupInteractionBuilder<F: Field> {
...
impl<F: Field> LookupInteractionBuilder<F> {
    /// Creates a new [`LookupInteractionBuilder`] by preparing preprocessed and main matrices of the given widths,
    /// initializing empty vectors of [`LookupInteraction`]s, and initializing an all-zero vector of public values.
    #[must_use]
    pub fn new(preprocessed_width: usize, main_width: usize) -> Self {
        let preprocessed_width = preprocessed_width.max(1);

        // Prepare the preprocessed matrix values based on the preprocessed width
        let prep_values =
            // Row indices
            [0, 1]
            .into_iter()
            // map each row index to an iterator containing the column values, and chain the iterators together
            .flat_map(|row| {
                // Column indices
                (0..preprocessed_width).map(move |column| {
                    SymbolicVariable::new(Entry::Preprocessed { offset: row }, column)
                })
            })
            .collect();

        // Prepare the main matrix values based on the main width
        let main_values = [0, 1]
            .into_iter()
            .flat_map(|row| {
                (0..main_width)
                    .map(move |column| SymbolicVariable::new(Entry::Main { offset: row }, column))
            })
            .collect();
    ...
    /// Consumes the builder and returns the lists of sent [`LookupInteraction`]s and received [`LookupInteraction`]s
    /// generated by evaluating an [`Air`](p3_air::air::Air) with this builder.
    ///
    /// If the builder was not mutated by a call to [`Air::eval()`](p3_air::air::Air::eval), these lists will be empty.
    #[must_use]
    pub fn interactions(self) -> (Vec<LookupInteraction<F>>, Vec<LookupInteraction<F>>) {
        (self.sends, self.receives)
    }
}

impl<F: Field> AirBuilder for LookupInteractionBuilder<F> {
    type F = F;
...
impl<F: Field> PairBuilder for LookupInteractionBuilder<F> {
    fn preprocessed(&self) -> Self::M {
        self.preprocessed.clone()
    }
}

impl<F: Field> MessageBuilder<AirInteraction<SymbolicExpression<F>>>
    for LookupInteractionBuilder<F>
{
    fn send(&mut self, message: AirInteraction<SymbolicExpression<F>>, scope: InteractionScope) {
        ...
        self.sends.push(LookupInteraction::new(values, multiplicity, message.kind, scope));
    }
    ...
    fn receive(&mut self, message: AirInteraction<SymbolicExpression<F>>, scope: InteractionScope) {
        ...
        self.receives.push(LookupInteraction::new(values, multiplicity, message.kind, scope));
    }
}

impl<F: Field> AirBuilderWithPublicValues for LookupInteractionBuilder<F> {
    type PublicVar = F;
...
mod tests {
    ...
    fn test_lookup_interactions() {
        ...
        let mut builder = LookupInteractionBuilder::<BabyBear>::new(0, NUM_COLS);
